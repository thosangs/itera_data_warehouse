...
# --- Register JDBC Sink to write CDC into Postgres staging tables ---
echo "[Day 9] Registering/Updating JDBC sink connector 'pg-sink-cdc'..."

SINK_CREATE_BODY=$(cat <<JSON
{
  "name": "pg-sink-cdc",
  "config": {
    ...
    "topics": "${DB_NAME}.source.customers,${DB_NAME}.source.orders",
    ...
    "transforms.route.regex": "^[^.]+\\.source\\.(.*)$",
    "transforms.route.replacement": "staging.$1_cdc",

    "primary.key.mode": "record_key",
    "table.primary.keys": "staging.customers_cdc:customer_id,staging.orders_cdc:order_id"
  }
}
JSON
)

SINK_UPDATE_BODY=$(cat <<JSON
{
  "connector.class": "io.debezium.connector.jdbc.JdbcSinkConnector",
  "tasks.max": "1",
  "topics": "${DB_NAME}.source.customers,${DB_NAME}.source.orders",
...
SINK_HTTP_CODE=$(echo "$SINK_CREATE_BODY" | curl -s -o /tmp/pg_sink_resp.txt -w "%{http_code}" -X POST "$CONNECT_URL/connectors" -H 'Content-Type: application/json' -d @- || true)
if [ "$SINK_HTTP_CODE" = "409" ]; then
  SINK_HTTP_CODE=$(echo "$SINK_UPDATE_BODY" | curl -s -o /tmp/pg_sink_resp.txt -w "%{http_code}" -X PUT "$CONNECT_URL/connectors/pg-sink-cdc/config" -H 'Content-Type: application/json' -d @- || true)
fi

if [ "$SINK_HTTP_CODE" != "200" ] && [ "$SINK_HTTP_CODE" != "201" ]; then
  echo "[Day 9] JDBC Sink registration failed (HTTP $SINK_HTTP_CODE). Response:" >&2
  cat /tmp/pg_sink_resp.txt >&2 || true
  exit 1
fi
echo "[Day 9] JDBC Sink up (HTTP $SINK_HTTP_CODE)."
